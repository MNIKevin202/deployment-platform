...
describe("appDatabase", () => {
  ...
  test("runs migrations and records applied versions", () => {
    ...
    assert.equal(rows[22].name, "app_deployment_retention");
    assert.equal(rows[23].version, 24);
    assert.equal(rows[23].name, "database_connections");
    ...
    assert.equal(rows[27].version, 28);
    assert.equal(rows[27].name, "platform_update_history");
  ...
  test("migrations are idempotent across reopen", () => {
    ...
    assert.equal(rows.length, 28);
    assert.equal(rows[rows.length - 1].version, 28);
    assert.equal(rows[rows.length - 1].name, "platform_update_history");
